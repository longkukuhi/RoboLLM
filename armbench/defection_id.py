...
from armbench.defection_datasets import create_armbench_dataset
...
from armbench.engine import train_one_epoch, get_handler, defection_evalate
...
def main(args, ds_init):
    ...
    if args.eval:
        ...
        exp_tag = f"{args.model}_{args.epochs}epochs_{args.seed}seed_{args.task}_tasks_{args.finetune.split('/')[-1]}/"
    ...
    if utils.is_main_process() and args.enable_wandb:
        ...
        wandb.init(project='armbench_defection',
    ...
    if args.eval:
        ...
        test_stats, task_key = defection_evalate(data_loader_val,
                                                 model=model,
        ...
        print(
            f"Accuracy of the network on the {len(data_loader_val.dataset)} test images: {test_stats[task_key]:.3f}%")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.eval_freq > 0 and (epoch + 1) % args.eval_freq == 0:
            ...
            if args.output_dir and utils.is_main_process():
                ...
                result_file_path = os.path.join(args.output_dir,
                                                f"{args.task}_{args.num_max_bpe_tokens}tokens_{args.finetune.split('/')[-1]}_test_results.txt")
