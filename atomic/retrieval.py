...
from beit3_tools.engine_for_finetuning import train_one_epoch, get_handler, evaluate
...
def main(args, ds_init):
    ...
    if not args.model.endswith(args.task):
        if args.task in ("flickr30k", "coco_retrieval", "atomic"):
    ...
    if utils.is_main_process() and args.enable_wandb:
        ...
        wandb.init(project='atomic_retrieval',
    ...
    if args.eval:
        ...
        if args.task in ["nlvr2", "flickr30k", "coco_retrieval", "imagenet", 'atomic']:
            ...
            json_file = f"output/{args.task}_{args.num_max_bpe_tokens}tokens_{''.join(data_loader_test.dataset.text_features)}_{args.finetune.split('/')[-1]}_test_results.json"
    ...
    utils.load_model_and_may_interpolate(best_model_path, model, args.model_key, args.model_prefix)
